...
public:
    ...
    float getAngle() const { return angle; }
		float getMyAngle() const { return My_angle;}
		uint16_t encoder_init(){return encoder;}
		int32_t get_round_cnt(){return round_cnt;}
    const uint8_t ID = 0;
	
    float Out = 0; /*!< Output ampere value that sent to ESC */
		int32_t total_encoder;
protected:
    float angle = 0,My_angle, encoder_offset = 0, last_encoder = 0;
    ...
    void update_angle(uint8_t can_rx_data[])
    {
      ...
      angle = total_encoder / ENCODER_ANGLE_RATIO();
			My_angle = encoder/ENCODER_ANGLE_RATIO();
    }
